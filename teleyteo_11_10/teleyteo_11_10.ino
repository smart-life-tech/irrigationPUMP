...
void processData(String inputString)
{
  ...
  if (found)
  {
    ...
    Serial.println(inputString);
    // inputString.toUpperCase(); // Uppercase the Received Message
    //  Serial.println(inputString.indexOf("#"));PROGNUBER#X#1234567890#
...
void errorpressure()
{
  Serial.println("Setting the GSM in text mode error pressure lower than 4 bar");
  ...
  {
    sendSMS(phoneNumbers[i], "low pressure!!");
    delay(5000); // Add a delay between sending SMS to each number (in milliseconds)
  }
}

/**
 * Sends SMS notifications to the phone numbers stored in the EEPROM
 * when watering has finished.
 *
 * This function sets the GSM in text mode for error stop watering,
 * reads the phone numbers from the EEPROM, and sends SMS notifications
 * to each phone number. It waits for 5 seconds between sending SMS to
 * each number.
 */
void errorStopWatering()
{
  // Print the GSM setup message
  Serial.println("Setting the GSM in text mode error stop watering");

  // Read the phone numbers from the EEPROM
  phoneNumbers[0] = readFromEEPROM(0);
  phoneNumbers[1] = readFromEEPROM(20);
  phoneNumbers[2] = readFromEEPROM(40);

  // Send SMS notifications to each phone number
  for (int i = 0; i < sizeof(phoneNumbers) / sizeof(phoneNumbers[0]); i++)
  {
    // Send SMS with message "watering has finished!"
    sendSMS(phoneNumbers[i], "watering has finished!");

    // Wait for 5 seconds before sending SMS to next number
    delay(5000);
  }
}
void errorDeviation()
...
void errorVoltage()
{
  ...
  for (int i = 0; i < sizeof(phoneNumbers) / sizeof(phoneNumbers[0]); i++)
  {
    sendSMS(phoneNumbers[i], "battery is low!!");
    delay(5000); // Add a delay between sending SMS to each number (in milliseconds)
  }
}
